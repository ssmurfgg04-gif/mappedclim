#!/usr/bin/env python3
"""v3 PROXY pipeline: XGB+LGB+CAT+ET+DART, H3-CV, 70/30 blend, stacking — target=proxy_simple_avg"""
...
print("=" * 70)
print("  v3 PIPELINE ON PROXY TARGET (proxy_simple_avg)")
print("=" * 70)
t0 = time.time()
...
# Drop list: original leakage cols + all proxy cols (target extracted before drop)
...
res_final = {
    'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'), 'pipeline': 'ultimate_v3_proxy', 'target': 'proxy_simple_avg',
...
print(f"Target: proxy_simple_avg | Leakage: County R2={leak['county_cv_r2']:.4f} -> H3 R2={leak['h3_cv_r2']:.4f}")
